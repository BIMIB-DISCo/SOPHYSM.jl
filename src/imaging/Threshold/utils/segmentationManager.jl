...
- `filepath_output::AbstractString`: The file path where the output files will 
  be saved.
...
"""
function apply_segmentation_SOPHYSM_tessellation(
    ...
    df_edges = DataFrame()
    df_labels, df_noisy_labels, df_total_labels = build_df_label(
        segments, min_threshold, max_threshold)
    # define centroids
    df_total_labels = compute_centroid_total_cells(
        segments, df_total_labels, min_threshold)
    df_labels = filter_dataframe_cells(df_total_labels, max_threshold)
    df_noisy_labels = filter_dataframe_extras(
        df_total_labels, min_threshold, max_threshold)
    # add column is_cell
    df_total_labels = add_column_is_cell(
        df_labels, df_noisy_labels, df_total_labels)
    # build tessellation
    ...
    df_edges, edges = build_graph_from_tessellation(
        df_labels, df_noisy_labels, df_total_labels, 
        width, height, filepath_total_tess, filepath_cell_tess)
    ...
    matrix = tess_dataframe_to_adjacency_matrix_weight(
        df_total_labels, df_edges, edges)
    ...
    # Use the same file as the segmented image
    filepath_background = filepath_output  
    ...
    # Read the segmented image as background
    img_graph = Luxor.readpng(filepath_background)  
    w = img_graph.width
    h = img_graph.height
    g_meta_total_labels = J_Space.spatial_graph(
        filepath_dataframe_edges, filepath_dataframe_total_labels)
    # Image with Vertices
    @png begin
        ...
        vertex_labels = [MetaGraphs.get_prop(g_meta_total_labels, v, :name) 
                         for v in MetaGraphs.vertices(g_meta_total_labels)]
        vertex_colors = extract_vertex_color(g_meta_total_labels)
        
        # Draw the graph with extracted information - use the underlying graph
        Karnak.drawgraph(
            # Create empty simple graph with the right number of vertices
            Graphs.SimpleGraph(MetaGraphs.nv(g_meta_total_labels)),
    ...
    @png begin
        ...
        Karnak.drawgraph(
            # Use the graph with edges instead of empty graph
            graph_with_edges,
            layout = vertex_positions .+ Karnak.Point(-w/2, -h/2),
            vertexlabels = vertex_labels,
            vertexfillcolors = vertex_colors
...
5. Visualizations of the graph with vertices and edges saved as
   "_graph_vertex.png" and "_graph_edges.png" in the output directory.
"""
function apply_segmentation_SOPHYSM_graph(
    filepath_input::AbstractString,
    filepath_output::AbstractString,
    thresholdGray::Float64,
    thresholdMarker::Float64,
    ...
    masked_colored_labels = colored_labels .* (1 .- bw)
    # build dataframe
    weight_fn(i,j) = euclidean(
        segment_pixel_count(segments,i), segment_pixel_count(segments,j))
    df_label = DataFrame()
    df_noisy_labels = DataFrame()
    df_total_labels = DataFrame()
    G, vert_map, df_label, df_noisy_labels, df_total_labels = 
        region_adjacency_graph_JHistint(
            segments, weight_fn, min_threshold, max_threshold)
    ...
    w = img_graph.width
    h = img_graph.height
    g_meta = J_Space.spatial_graph(
        filepath_dataframe_edges, filepath_dataframe_labels)

    # Verifica che il grafo sia stato costruito correttamente
    if MetaGraphs.nv(g_meta) == 0
        error("Il grafo costruito è vuoto. Verifica i dati di input e la " *
    ...
    for v in MetaGraphs.vertices(g_meta)
        if !haskey(g_meta.vprops[v], :centroid)
            error("Il vertice $v non contiene la proprietà :centroid. " *
                  "Verifica la costruzione del grafo.")
        end
    end
    ...
    @png begin
        ...
        vertex_labels = [MetaGraphs.get_prop(g_meta, v, :name) 
                         for v in MetaGraphs.vertices(g_meta)]
